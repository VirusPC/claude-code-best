# query() 的故事：从 ReAct 到 Claude Code 完整循环

> 用 **3 层扩展** 讲清 `src/query.ts`：最普通的 LLM ⇄ Tool ReAct → 加上生产壳 → 完整 `queryLoop`。
>
...
**内核永远是 ReAct**；Claude Code 在 LLM 节点前后各包了一层「上下文壳」和「产品壳」，没有改 ReAct 的两节点结构。
...
## Layer 1 扩展：上下文壳（Pre-LLM）

### 为什么需要这一层

纯 ReAct 假设 `messages` 永远能塞进模型窗口。长会话里会在 **第一次 LLM 调用前** 就 413 或触发 blocking limit。

Layer 1 在 **每次 iteration 调 LLM 之前** 先整理 `messagesForQuery`，ReAct 环不变。
...
当 **关闭 auto-compact** 且 token 已达硬上限时，**不调 LLM**，直接 `Terminal: blocking_limit`——这是 Layer 1 唯一在 ReAct 环外提前结束的常见路径。
...
- **ReAct 没变**：还是 `prep → llm → tools → prep → …`
- **新增的是 prep 节点**：把「能发给 API 的消息」准备好
- 源码范围：约 `query.ts:522–873`（`context_prep` 段）

---

## Layer 2 扩展：执行壳（Post-LLM / Post-Tools）

Layer 1 解决「发不出去」；Layer 2 解决「发出去了但坏了」和「tool 跑完后还要塞东西」。

### 2a — LLM 之后的错误与恢复（仍无 tool 时）
...
恢复成功 = **改 `state.messages` 后 `continue`**，回到 Layer 1 的 `prep`，再进 LLM——仍是 ReAct，只是 messages 被换过。

### 2b — Tool 路径上的执行与注入

有 `tool_use` 时走 tools；结束后 **不立刻** 回到 llm，中间多一步 **attachments**：

```mermaid
flowchart LR
  prep --> llm
  llm -->|tool_use| tools
  llm -->|无 tool| finish["finish"]
...
| 步骤 | 作用 |
|------|------|
| `runTools` / `StreamingToolExecutor` | 权限、执行、yield tool_result |
| abort mid-tools | `aborted_tools` |
| `hook_stopped_continuation` | `hook_stopped` |
| `getAttachmentMessages` | 队列命令、文件变更等 |
| memory / skill / tool prefetch | 异步预取结果注入 |
...
然后 `messages = messagesForQuery + assistant + toolResults`，`transition: next_turn`，进入下一轮 prep。

### 2c — 流式并行（不改变图，只改 tools 实现）

`StreamingToolExecutor`：LLM 还在 stream 时就开始跑 tool，**节点仍是 llm + tools**，只是 tools 与 llm 时间重叠。

### Layer 2 小结

- **ReAct 仍是两节点**，但 tools 后多了 **attachments** 子步骤
- **finish** 分支在「无 tool」时处理 recovery，成功则跳回 **prep**（不是跳回 llm，会先再过一遍 compact）
- 源码：recovery `1334–1528`；tools `1635–1683`；attachments `1826–2040`

---

## Layer 3 扩展：产品壳（Turn 结束 & 进程级）

### 3a — Turn 结束：stop hooks & token budget
...
- **不改变 llm ⇄ tools**，只在「无 tool 的一轮结束」和「进程退出」上加规则
- `Continue` = 带着改过的 `messages` 回到 **prep**（进入下一轮 `while` iteration）
- `Terminal` = 整次 `query()` 结束，交给 REPL / SDK
...
```mermaid
flowchart TB
  subgraph L0["Layer 0 内核 — ReAct"]
    llm["🤖 llm<br/>deps.callModel"]
    tools["🔧 tools<br/>runTools / StreamingToolExecutor"]
    llm -->|"tool_use"| tools
    tools --> llm
  end

  subgraph L1["Layer 1 — 上下文壳 prep"]
    prep["prep<br/>snip → microcompact → collapse → autocompact"]
    prep --> llm
    tools --> attach
    attach["attachments<br/>queue / memory / skill prefetch"]
    attach --> prep
  end

  subgraph L2["Layer 2 — 执行壳"]
    finish["finish<br/>PTL / max_output recovery"]
    llm -->|"无 tool"| finish
    finish -->|"recovery ok"| prep
    finish -->|"done / fatal"| END_NODE((END))
  end

  subgraph L3["Layer 3 — 产品壳"]
    hooks["stop_hooks + token_budget"]
    finish --> hooks
    hooks -->|"completed"| END_NODE
    hooks -->|"blocking / budget"| prep
    wrap["query() wrapper<br/>Langfuse / autonomy / finally"]
  end
...
| 层 | 解决的问题 | 主要节点/代码 | 是否改变 ReAct |
|----|------------|---------------|----------------|
| **0** | 模型推理 + 工具循环 | `callModel` ⇄ `runTools`，`needsFollowUp` | — 这就是 ReAct |
| **1** | 上下文塞不进窗口 | prep：`522–873`，`autocompact` | 否，只包在 llm 前 |
| **2** | API 失败恢复、tool 后注入 | recovery `1334–1528`，attachments `1826–1985` | 否，finish/attach 是边 |
| **3** | hooks、预算、中断、观测 | `handleStopHooks`，`query()` finally | 否，turn/进程级 |

### 单次用户消息的生命周期（叙事）

1. **REPL** 把用户输入交给 `query(messages, …)`（见 [startup-to-query-walkthrough](./startup-to-query-walkthrough.md)）。
2. **`query()`** 建 Langfuse trace，进入 **`queryLoop` 的 `while (true)`**。
3. **Layer 1 — prep**：把当前 `messages` 压到能发 API；可能 yield compact 摘要。
4. **Layer 0 — llm**：流式调用；有 `tool_use` 则 `needsFollowUp = true`。
5. **若无 tool — Layer 2 finish + Layer 3 hooks**：尝试 withhold 错误恢复；跑 stop hooks / token budget；`completed` 或 continue。
6. **若有 tool — Layer 0 tools → Layer 2 attachments**：执行工具、注入附件，拼回 `messages`，`next_turn`。
7. 回到步骤 3，直到 **Terminal** 或 **maxTurns**。
8. **`query()` finally**：autonomy 收尾、flush trace，返回 `Terminal` 给 REPL。
...
**可学**：任何 agent 实现都先固定 `llm ⇄ tools`，其它一律标成「进 LLM 前 / 出 LLM 后 / turn 结束」的壳。

**好处**：

- Code review 时问「这条逻辑改的是 ReAct 还是 prep？」——改 ReAct 要极其谨慎。
- 换框架（LangGraph、自建 loop）时，**壳可以拆成 middleware**，内核仍是两个 handler。
- 新人不会被 snip / reactive compact 吓到：它们不属于「第三种节点」，只是 prep 里的步骤。

**反例**：把 compact、recovery、hooks 都建成平等节点，图好看但 **架构真相被淹没**，后续每加功能就多一个节点，环变毛球。
...
| 上下文爆了怎么办？ | prep pipeline，主动 + 被动 recovery |
| 用户中断怎么办？ | 分 `aborted_streaming` / `aborted_tools`，streaming 时合成 tool_result |
| 怎么避免 hook 放大故障？ | API 无效响应不跑 stop hooks |
| 怎么测路径？ | `transition.reason` + `QueryDeps` mock |
| 怎么保持可读？ | 三层壳叠在 ReAct 上，架构图始终两节点 |
